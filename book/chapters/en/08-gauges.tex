...
$A \in \Lie$, the Lie algebra of $G$, the \emph{fundamental vector field} is
\[
...
V_u P$, and the map $A \mapsto A^*_u$ is a canonical isomorphism $\Lie \cong
V_u P$.
\end{definition}
...
$\omega \in \Omega^1(P, \Lie)$ such that
\begin{enumerate}
  \item \emph{normalization}: $\omega(A^*_u) = A$ for all $A \in \Lie$;
...
The \emph{curvature} of a connection $\omega$ is the $\Lie$-valued $2$-form
\[
